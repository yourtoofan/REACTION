        ...
        for member in message.new_chat_members:
            ...
            if member.id == client.id:
                try:
                    ...
                    await message.reply_photo(photo=random.choice(IMG), caption=START.format(await client.get_me()).mention or "can't mention", users, chats), reply_markup=reply_markup)
...
async def start(client: Client, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(await client.get_me()).mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
...
async def ping(client: Client, message: Message):
    ...
    await loda.edit_text(
        text=f"нey вαву!!\n{client.name} ᴄʜᴀᴛʙᴏᴛ ιѕ alιve 🥀 αnd worĸιng ғιne wιтн a pιng oғ\n\n**➥** `{ms}` ms\n**➲ ᴄᴘᴜ:** {CPU}\n**➲ ʀᴀᴍ:** {RAM}\n**➲ ᴅɪsᴋ:** {DISK}\n**➲ ᴜᴘᴛɪᴍᴇ »** {UP}\n\n<b>||**๏ мαdє ωιтн ❣️ ву [ᴠɪᴘ ʙᴏʏ](https://t.me/{OWNER_USERNAME}) **||</b>",
